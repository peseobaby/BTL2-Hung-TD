        ...
        if (Hash::check($password, Auth::user()->password)) {
            ...
            if (($userIn != Null) && ($request->name == $userIn->user->name)) {
                ...
                if((($purseFromMoney - $request->money) > ZERO) && ($request->money > ZERO)) {
                    ...
                    $category = is_null($request->category) ? $category = Category::find(expense) : Category::where('name', $request->category)->first();
                    ...
                    return redirect()->route('purse.trade', ['id' =>$id])->with('alert', 'Số tiền cần chuyển phải nhỏ hơn số tiền đang có và lớn hơn 0');
                } 
            } else {
                return redirect()->route('purse.trade', ['id' =>$id])->with('alert', 'Sai tên người nhận hoặc tài 
                khoản');
            }
        } else {
                return redirect()->route('purse.trade', ['id' =>$id])->with('alert', 'Sai mật khẩu');
        }
   		   return redirect()->route('purse', ['purse' => $purseFrom, 'id' => $purseFrom->user_id])->with('alert', 'Chuyển tiền thành công');
   	}
}
